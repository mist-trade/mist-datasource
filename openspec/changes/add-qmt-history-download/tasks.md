...
- [x] 1.2 `[mist-datasource]` 路由：`POST /v1/qmt/download`（校验：
  symbol 格式、base_periods ⊆ {1m,5m,1d}、≤64 标的、日期合法；ActivityWindow
  门禁 → `in_session`）+ `GET /v1/qmt/download/{jobId}`（逐任务+聚合）。
...
- [x] 1.4 `[mist-datasource]` 可观测性：job 计数器
  ...
  warn（reason 有界）。
- [x] 1.5 `[mist-datasource]` 配置：`QMT_DOWNLOAD_JOB_ENABLED`（datasource，
...
- [x] 3.1 `[mist]` 统计缺失：QMT 源标的 × 基础周期(1m/5m/1d) × 目标窗查 k 表，
  枚举缺口（跳过已完整窗口）。
- [x] 3.2 `[mist]` 下载提交与轮询：缺口 → `POST /v1/qmt/download` 提交 job
  → 轮询 status（5–10s 间隔，预算 ≤ 600s，超时走既有 notReady 路径）。
- [x] 3.3 `[mist]` 编排挂接：夜间/晨间同步与手动 collect 前置该三段（TDX 源
  标的不提交——refresh 由 datasource get_bars 内置）。
- [x] 3.4 `[mist tests]` 单测：缺口枚举、提交/轮询（完成/超时/拒绝）、TDX 跳过。
...
- [x] 4.2 `[deploy]` 部署 datasource 容器 + backend 容器（编排上线）。
...
- [x] 5.2 `[mist-datasource]` **TDX 下载端点**：`POST /v1/tdx/download`（`{stock_list, base_periods ⊆ {1m,5m,1d}}`）→ 逐周期 `refresh_kline` → 逐周期状态返回；symbol/period 校验前置；同步限时（实测 ~100ms，预算 600s）；`raise_for_native_error` 适配 `ErrorId==0` 成功形状（实测 `Msg` 字段）。
